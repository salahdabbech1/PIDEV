 ...
 * @author HP
 */
public class ModifierMovieForm extends BaseForm {
    Form current;
    public ModifierMovieForm(Resources res,Movie m){
...
TextField imageb = new TextField((m.getImageb()), "imageb", 20, TextField.ANY);
    TextField director = new TextField((m.getDirector()), "director", 20, TextField.ANY);
       Picker dateR=new Picker();
       dateR.setUIID("NewsTopLine");
       imageb.setUIID("NewsTopLine");
       director.setUIID("NewsTopLine");
...
image.setSingleLineTextArea(true);
    ...
    m.setRdate(dateR.getDate());
    m.setTrailer(trailer.getText());
    m.setDirector(director.getText());
    m.setGenre(p.getValue().toString());
    
    
     try{
           if(ServiceMovie.getInstance().modifierMovie(m)){
    ...
    e,a,new FloatingHint (name),
            ...
            createLineSeparator(),new FloatingHint (director),
