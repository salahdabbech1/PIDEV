 ...
 * @author HP
 */
public class ListTactorForm extends BaseForm{
    Form current;
    private EncodedImage placeHolder;
    public ListTactorForm(Resources res) {
...
RadioButton liste = RadioButton.createToggle("LISTE DES TACTORS", barGroup);
...
updateArrowposition(barGroup.getRadioButton (barGroup.getSelectedIndex()), arrow);
 ...
 Form f=new Form("Form");
   ...
   new AjoutTactorForm(res).show();

});
 f.addAll(ret,ajt);
 add(f);
    
        ArrayList<Tactor> list = ServiceTactor.getInstance().AffichageTactor();
...
//        Label ta3 = new Label("Votre description :"+u.getDescription());
        ...
        else
        {
            dig.dispose();
       try{
           if(ServiceTactor.getInstance().deleteTactor(u.getId()))
        {
       ...
       new ListTactorForm(res).show();
        ...
        lModifier.addPointerPressedListener(l->{
           ...
           new ModifTactorForm(res,u).show();
