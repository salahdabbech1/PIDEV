 ...
 * @author HP
 */
public class ListFactorForm extends BaseForm{
    Form current;
    private EncodedImage placeHolder;

    public ListFactorForm(Resources res) {
...
RadioButton liste = RadioButton.createToggle("LISTE DES FACTORS", barGroup);
...
updateArrowposition(barGroup.getRadioButton (barGroup.getSelectedIndex()), arrow);
   ...
   new AjoutFactorForm(res).show();

});
 f.addAll(ret,ajt);
 add(f);

        ArrayList<Factor> list = ServiceFactor.getInstance().AffichageFactor();
    ...
    private void addButton(Factor u,Resources res) {
      ...
      //  Label ta4 = new Label("Votre status :"+u.getImage());
        ...
        supprimer.addPointerPressedListener(l-> {
                ...
                else
                {
                    dig.dispose();
               try{
                   if(ServiceFactor.getInstance().deleteFactor(u.getId()))
                {
        ...
        FontImage modifierImage=FontImage.createMaterial(FontImage.MATERIAL_EDIT, modifierstyle);
         ...
         lModifier.setTextPosition(LEFT);
            ...
            lModifier.addPointerPressedListener(l->{
               ...
               new ModifierFactorForm(res,u).show();
