 ...
 * @author HP
 */
public class IndexMovieForm extends BaseFront{
    Form current;
    private EncodedImage placeHolder;
    public IndexMovieForm(Resources res) {
...
Container f=new Container();
 ...
 add(f);
        ...
        ArrayList<Movie> list = ServiceMovie.getInstance().AffichageMovie();
...
details.addPointerPressedListener(l->{
   ...
   new showMovieForm(res,u).show();

});
 Button Share=new Button("Share");
 //Yosri 7ot houni win thezzek
 Share.addPointerPressedListener(l->{
     Display.getInstance().execute("https://www.facebook.com/sharer/sharer.php?kid_directed_site=0&sdk=joey&u=http%3A%2F%2F127.0.0.1%3A8000%2Ffilm%2F"+u.getId()+"&display=popup&ref=plugin&src=share_button");

  // new showMovieForm(res,u).show();

});

        f.addAll(imavu,ta,details,Share);
        add(f);
