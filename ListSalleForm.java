 ...
 * @author HP
 */
public class ListSalleForm extends BaseForm{
    Form current;
    private EncodedImage placeHolder;

    public ListSalleForm(Resources res) {
        //   super("salle", BoxLayout.x());
...
updateArrowposition(barGroup.getRadioButton (barGroup.getSelectedIndex()), arrow);
        ...
        ArrayList<salle> list = Servicesalle.getInstance().Affichagesalle();
    ...
    private void addButton(salle u,Resources res) {

        Container cnt=new Container();
        Form f =  new Form("Form", BoxLayout.y()); 
        Label tt = new Label("******salle******");
        Label ta = new Label("Id cinema :"+u.getId_cine());
        Label ta2 = new Label("nombres des chaise :"+u.getNb_chaise());
...
supprimer.addPointerPressedListener(l-> {
    ...
    else
    {
        dig.dispose();
   try{
       if(Servicesalle.getInstance().deletesalle(u.getId()))
    {
...
lModifier.addPointerPressedListener(l->{
  ...
  // new ModifierSalleForm(res,u).show();

});


        f.addAll(tt,ta,ta2,supprimer,lModifier);
        add(f);
